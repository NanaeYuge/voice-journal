...
function RecordingModal({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const whisperRes = await fetch("/api/analyze", {
        method: "POST",
        body: formData,
      });
      const whisperData = await whisperRes.json();

      // journals に保存（source: timecapsule, linked_journal_id: 元のID）
      await supabase.from("journals").insert({
        user_id: userData.user.id,
        transcript: whisperData.transcript || "",
        emotion: whisperData.emotion || "穏やか",
    ...
    <>
      <style>{`
        ...
        .capsule-ai-loading {
          ...
          animation: logs-fade 2s ease-in-out infinite;
          margin-bottom: 8px;
        }
        .capsule-prompt {
          font-size: 12px;
        ...
        .modal-divider {
          border: none; border-top: 1px solid rgba(255,255,255,0.05); margin: 16px 0;
        }
        .modal-prompt {
          font-size: 14px; color: rgba(255,255,255,0.65);
          letter-spacing: 0.06em; text-align: center; margin: 0 0 24px 0;
          font-family: 'Zen Old Mincho', serif;
        }
        ...
        .modal-rec-btn.recording {
          border-color: rgba(244,132,106,0.5);
          background: rgba(244,132,106,0.08);
          color: rgba(244,132,106,0.9);
          animation: pulse 1.5s ease-in-out infinite;
        }
        @keyframes pulse { 0%,100%{box-shadow:0 0 0 0 rgba(244,132,106,0.2)} 50%{box-shadow:0 0 0 8px rgba(244,132,106,0)} }
        .rec-dot {
          width: 8px; height: 8px; border-radius: 50%;
          background: rgba(244,132,106,0.9);
        }
        ...
        .modal-btn-primary {
          ...
          background: rgba(139,92,246,0.1); color: rgba(167,139,250,0.9);
          font-size: 13px; font-family: 'Noto Sans JP', sans-serif;
          letter-spacing: 0.1em; cursor: pointer; transition: all 0.2s;
        }
        .modal-btn-primary:hover { background: rgba(139,92,246,0.18); }
        ...
        .modal-btn-secondary:hover { color: rgba(255,255,255,0.4); }
        .modal-close-btn {
        ...
        .modal-saved {
          ...
          animation: summary-in 0.5s ease forwards;
        }
        .modal-saved span { font-size: 28px; display: block; margin-bottom: 12px; }
        .modal-saved p {
          font-size: 13px; color: rgba(167,139,250,0.75);
          letter-spacing: 0.06em; line-height: 1.8; margin: 0;
        }
      `}</style>
